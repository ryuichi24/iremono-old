...
import { selectedCurrentCryptoFolderSelector } from '@/store/selected/selected-slice';
...
export const UnlockedCryptoFolders = () => {
  ...
  const selectedCurrentCryptoFolder = useAppSelector(selectedCurrentCryptoFolderSelector);
  const currentFolderGroup = useAppSelector((state) => folderGroupByIdSelector(state, selectedCurrentCryptoFolder?.id));
  const currentFileGroup = useAppSelector((state) => fileGroupSelectedById(state, selectedCurrentCryptoFolder?.id));
  ...
  useEffect(() => {
    (async () => {
      const currentFolder = await foldersService.get({ folderId: currentFolderId, folderType: 'crypto' });
      setSelectedCurrentCryptoFolder({ selectedCurrentCryptoFolder: currentFolder });
  ...
  const handleUploadChange: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    const result = await filesService.upload({
      parentId: selectedCurrentCryptoFolder?.id,
    ...
    addOneFileItem({ fileItem: result, parentId: selectedCurrentCryptoFolder?.id });
    e.target.value = '';
  };
  ...
  return (
    <Container>
      <Header isSubHeader={true}>
        <>
          <FolderPathNav currentFolder={selectedCurrentCryptoFolder} />
        </>
        <>
      ...
      <NewFolderForm
        ...
        currentFolderId={selectedCurrentCryptoFolder?.id}
